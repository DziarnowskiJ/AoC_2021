...
with open(base_path + f'/inputs/sample/sample_input_day_18_5.txt', 'r') as file:
...
def process(lines):
    ...
    permutations = [[i, j] for i in numbers for j in numbers if i != j]

    max_mag = 0
    for permutation in permutations:
        number = deepcopy(permutation)
        max_mag = max(get_magnitude(reduction(number)), max_mag)
