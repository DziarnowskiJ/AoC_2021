...
def calculate_packets(packets):
    version, id, packet = packets
    if id == '0':
        return sum([calculate_packets(p) for p in packet])
    elif id == '1':
        return reduce(mul, [calculate_packets(p) for p in packet])
    elif id == '2':
        return min([calculate_packets(p) for p in packet])
    elif id == '3':
        return max([calculate_packets(p) for p in packet])
    elif id == '4':
        return packet
    elif id == '5':
        return 1 if calculate_packets(packet[0]) > calculate_packets(packet[1]) else 0
    elif id == '6':
        return 1 if calculate_packets(packet[0]) < calculate_packets(packet[1]) else 0
    elif id == '7':
        return 1 if calculate_packets(packet[0]) == calculate_packets(packet[1]) else 0
...
def process(lines):
    ...
    result = calculate_packets(packets)
    return result
